...
{
	public class RGSetInitialViewInfo : TraitInfo<RGSetInitialView>
	{
		public readonly int InitialExploreRange = 5;
	}

	public class RGSetInitialView : IWorldLoaded
	{
		...
		public void WorldLoaded(World world)
		{
			...
			foreach (Session.Slot slot in world.LobbyInfo.Slots)
			{
				...
				Session.Client client = world.LobbyInfo.Clients.FirstOrDefault(c => c.Slot == slot1.Index);
			...
			foreach (var p in Start)
				if (p.Key == world.LocalPlayer || p.Key.Stances[world.LocalPlayer] == Stance.Ally)
					world.WorldActor.Trait<Shroud>().Explore(world, p.Value,
															 world.WorldActor.Info.Traits.Get<RGSetInitialViewInfo>().
																InitialExploreRange);

			// Set viewport
			if (Start.ContainsKey(world.LocalPlayer))
			{
