...
{
	public class RGIonBeaconTargetInfo : ITraitInfo
	{
		#region ITraitInfo Members

		public object Create(ActorInitializer init)
		{
			return new RGIonBeaconTarget(init.self);
		}

		#endregion
	}

	public class RGIonBeaconTarget : ITick, IRGPostRender, IRGBeaconTarget
	{
		...
		public Actor Self;
		public int TicksBeforeLaunch { get; protected set; }
		public int TotalTicksBeforeLaunch;

		public RGIonBeaconTarget(Actor self)
		{
			Self = self;
		}
		...
		public void Tick(Actor self)
		{
			...
			{
				RGGame.EVA("Ion Cannon approaching!");
		...
		public void DoDisarm(Player disarmer, float amount)
		{
			if (DisarmProgress < 1f)
			{
				...
				if (DisarmProgress >= 1f)
				{
					...
					RGGame.EVA("Ion Beacon disarmed!");
				}
			}
		}
		...
		public void TriggerLaunch(Player launcher, Actor launchSite, int ticks)
		{
			...
			Game.AddChatLine(Color.OrangeRed, "EVA",
			                 "{0} has placed an Ion Cannon Beacon on the {1}.".F(launcher.PlayerName, 
