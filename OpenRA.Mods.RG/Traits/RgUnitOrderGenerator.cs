...
namespace OpenRA.Mods.Rg.Traits
{
	...
	internal class RgUnitOrderGenerator : ICustomUnitOrderGenerator
	{
		...
		private static UnitOrderResult OrderForUnit(Actor self, int2 xy, MouseInput mi, Actor underCursor)
		{
            ...
            */
			...
			if (mi.Button == MouseButton.Right)
			{
				...
				{
					List<Actor> actorsAt = uim.GetUnitsAt(xy).ToList();

					string cursor = null;
					if (underCursor != null)
						if (o.Order.CanTargetUnit(self, underCursor, mi.Modifiers.HasModifier(Modifiers.Ctrl),
						                          mi.Modifiers.HasModifier(Modifiers.Alt), ref cursor))
							return new UnitOrderResult(self, o.Order, o.Trait, cursor, Target.FromActor(underCursor));
					if (o.Order.CanTargetLocation(self, xy, actorsAt, mi.Modifiers.HasModifier(Modifiers.Ctrl),
					                              mi.Modifiers.HasModifier(Modifiers.Alt), ref cursor))
