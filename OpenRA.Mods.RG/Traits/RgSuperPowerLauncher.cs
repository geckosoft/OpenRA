...
namespace OpenRA.Mods.Rg.Traits
{
	...
	public class RgSuperPowerLauncher : IIssueOrder, IResolveOrder, IOrderVoice
	{
		...
		{
			if (Ammo > 0 && order.OrderString == "PlaceBeacon")
				// && order.TargetActor.Owner.Stances[order.Subject.Owner] == Stance.Enemy) /* only target enemies */
			{
		...
		#region Nested type: RgSuperPowerLauncherOrderTargeter

		private class RgSuperPowerLauncherOrderTargeter : UnitTraitOrderTargeter<Building>
		{
