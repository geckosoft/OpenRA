...
{
	internal class RgIonBeaconTargetInfo : ITraitInfo
	{
	...
	internal class RgIonBeaconTarget : ITick, IRgPostRender
	{
		...
		public void DoDisarm(Player disarmer, float amount)
		{
			if (DisarmProgress < 1f)
			{
				...
				if (DisarmProgress >= 1f)
				{
					...
					Game.AddChatLine(Color.OrangeRed, "EVA", "Ion Beacon disarmed!");
				}
			}
		}
