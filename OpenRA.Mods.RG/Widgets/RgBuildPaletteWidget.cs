...
namespace OpenRA.Mods.Rg.Widgets
{
    ...
    class RgBuildPaletteWidget : Widget
    {
        public int Columns = 3;
        public int Rows = 5;

        RgProductionQueue CurrentQueue = null;
        List<RgProductionQueue> VisibleQueues = new List<RgProductionQueue>();

        public bool paletteOpen = false;
        ...
        public bool paletteAnimating = false;
        ...
        public RgBuildPaletteWidget([ObjectCreator.Param] World world)
        {
            this.world = world;
        }
        ...
        public override void Tick()
        {
            ...
            var queues = world.Queries.WithTraitMultiple<RgProductionQueue>()
                .Where(p => p.Actor.Owner.Stances[world.LocalPlayer] == Stance.Ally && p.Actor.Owner.PlayerRef.OwnsWorld && !p.Actor.Owner.PlayerRef.NonCombatant)
                .Select(p => p.Trait);
        ...
        int DrawPalette(WorldRenderer wr, World world, RgProductionQueue queue)
        {
            buttons.Clear();
            ...
            var myUnits = world.Queries.OwnedBy[world.LocalPlayer].Where(a => !a.Destroyed  && a.IsInWorld && a.Info.Name != "c17" && a.Owner == world.LocalPlayer && a != a.Owner.PlayerActor).ToList();
        ...
        Action<MouseInput> HandleTabClick(RgProductionQueue queue, World world)
        {
            return mi =>
            {
